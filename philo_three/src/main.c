...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/24 09:14:57 by lrobino           #+#    #+#             */
/*   Updated: 2021/05/24 09:15:23 by lrobino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	rules.die_time = ft_atoui(av[2]);
	rules.eat_time = ft_atoui(av[3]);
	rules.sleep_time = ft_atoui(av[4]);
	rules.has_max_eat = 0;
	if (ac == 6)
	{
		rules.max_eat = ft_atoui(av[5]);
		rules.has_max_eat = 1;
	}
	else
		rules.max_eat = 0;
	return (check_rules (rules, av, ft_atoui(av[1]))
		|| init_table(rules, ft_atoui(av[1])));
}
