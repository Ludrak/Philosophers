...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/24 09:16:06 by lrobino           #+#    #+#             */
/*   Updated: 2021/05/24 09:17:47 by lrobino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (sem_wait(g_table->forks_counter) != 0
		|| philo_talk(philo, A_FORK, 1) == EXIT_FAILURE)
	{
...
uint8_t	philo_eat(t_philo *const philo)
{
	...
	if (sem_post(g_table->forks_counter) != 0
		|| sem_post(g_table->forks_counter) != 0)
		return (EXIT_FAILURE);
	return (EXIT_SUCCESS);
}
